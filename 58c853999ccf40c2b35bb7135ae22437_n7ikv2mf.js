...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let body = {};
    let header = { ytenantid: "youridHere" };
    ...
    let updPartOutResouceList = [];
    let newConDetailList = [];
    if (datas && datas.length > 0) {
      for (var i = 0; i < datas.length; i++) {
        ...
        if (noOrgList.includes(org_name) || noOrgList.includes(first_party_name)) {
          continue;
        }
        if (billstatus != "19" && billstatus != "20") {
          continue;
        }
        let iserp = "N";
        if (billstatus == "19") {
        ...
        if (productline !== "" && productline !== null && !proArray.includes(productline)) {
          continue;
        }
        if (cglb == "华为产业云服务采购" || cglb == "APICloud外包") {
          continue;
        }
        var porRes = ObjectStore.selectByMap("AT17E908FC08280001.AT17E908FC08280001.part_out_resouce", { part_contract_code: data.billno, part_is_pre_invest2: "N" });
        if (porRes && porRes.length > 0) {
          let updPartOutResouce = {
            id: porRes[0].id,
            part_create_date: approve_end_time,
            part_is_erp: iserp
          };
          updPartOutResouceList.push(updPartOutResouce);
          continue;
        }
        ...
        let part_out_resouce = {
          ...
          part_pro_month: "202310", //formatMonth(newBeiJingDate()),
        ...
        let newConDetail = {
          purchase_code: data.billno,
          purchase_amount: data.contractmoney
        };
        newConDetailList.push(newConDetail);
      }
    }
    if (newPartOutResouceList.length > 0) {
      ObjectStore.insertBatch("AT17E908FC08280001.AT17E908FC08280001.part_out_resouce", newPartOutResouceList, "ybd993b5aa");
    }
    if (updPartOutResouceList.length > 0) {
      ObjectStore.updateBatch("AT17E908FC08280001.AT17E908FC08280001.part_out_resouce", updPartOutResouceList, "ybd993b5aa");
    }
    if (newConDetailList.length > 0) {
      ObjectStore.insertBatch("AT17E908FC08280001.AT17E908FC08280001.yccontract_detail", newConDetailList, "yb1e1b6db5");
    }
    return { newPartOutResouceList, updPartOutResouceList, newConDetailList, url, ycdatas };
  }
}
