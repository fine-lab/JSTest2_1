...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (var i = 0; i < datas.length; i++) {
      ...
      var recBuDetObjList = datas[i].recBuDetObjList;
      for (var j = 0; j < recBuDetObjList.length; j++) {
        makebodyinfo(recBuDetObjList[j]);
        var recBuPlanObjList = recBuDetObjList[j].recBuPlanObjList;
        for (var k = 0; k < recBuPlanObjList.length; k++) {
          makeSoninfo(recBuPlanObjList[k]);
        }
      }
    }
    let returns = ObjectStore.insert("AT175542E21C400007.AT175542E21C400007.recBusObject", datas, "rectrbuobject");
    let param = { return: returns };
    let func = extrequire("AT175542E21C400007.backDesignerFunction.businobjrecsave");
    ...
    function makeheadinfo(datahead) {
      ...
      datahead.customer = queryCustomerID(datahead);
    ...
    function makebodyinfo(databody) {
      ...
      databody.orgExchangeRateType = queryaccExchangeRateTypeID(databody.orgExchangeRateType);
      databody.expenseItem = queryaccexpenseitemID(databody.expenseItem);
      databody.unit = queryaccmaterialUnitID(databody.unit);
      databody.financeOrg = queryFinanceOrgID(databody);
      ...
      databody.receiveAgreement = queryReceiveAgreementID(databody.receiveAgreement);
    ...
    function makeSoninfo(datason) {
      ...
      datason.settleMode = querySettleMethodID(datason.settleMode);
      datason.arSubject = queryAccSubjeID(datason.arSubject);
    }
    //项目
    ...
    //收款协议
    function queryReceiveAgreementID(code) {
      let sql = "select * from 		bd.receivables.ReceiveAgreement	 where code='" + code + "'";
      var res4 = ObjectStore.queryByYonQL(sql, "ucfbasedoc");
      var FinanceOrgID = "";
      if (res4.length > 0) {
        FinanceOrgID = res4[0].id;
    ...
    //结算方式
    function querySettleMethodID(code) {
      let sql = "select * from 		aa.settlemethod.SettleMethod	where isEnabled=1  and   code='" + code + "'";
    ...
    //查询客户主键
    function queryCustomerID(datahead) {
      let sql = "select * from 			aa.merchant.Merchant where   code='" + datahead.customer + "'";
      var res4 = ObjectStore.queryByYonQL(sql, "productcenter");
      var FinanceOrgID = "";
      if (res4.length > 0) {
        FinanceOrgID = res4[0].id;
