let rukubustype = "1777834076088565763"; //其他入库交易类型
let chukubustype = "1780276091410710536"; //其他出库交易类型
viewModel.get("button28hj") &&
  viewModel.get("button28hj").on("click", function (data) {
    //作废--单击
    let othindata = viewModel.getAllData();
    let othinmainid = othindata.othinmainid;
    if (!isEmpty(othinmainid)) {
      let queryres = cb.rest.invokeFunction("AT18E3E0443F380007.afterapi.quOthinAudit", { othinmainid }, function (err, res) {}, viewModel, { async: false });
      if (queryres.hasOwnProperty("result") && queryres.result.hasOwnProperty("auditres") && queryres.result.auditres.length > 0) {
        ...
        let result = cb.rest.invokeFunction("AT18E3E0443F380007.afterapi.aftecancel", { othindata: othindata, rewriteorcancel: "CANCEL" }, function (err, res) {}, viewModel, { async: false });
...
function isCheck(data) {
  ...
  let sum = 1;
  if (data.hasOwnProperty("applyOthInRecordsList")) {
    let viewrecords = data.applyOthInRecordsList;
...
function getOthInRecords(clonedata) {
  let viewrecords = clonedata.applyOthInRecordsList;
  ...
  if (viewrecords.length > 0) {
    for (let i = 0; i < viewrecords.length; i++) {
      ...
      let costpriceresult = cb.rest.invokeFunction(
        "ST.otherOutRecord.queryCostprice",
...
function saveOthin(applydata, childs, status, flag) {
  ...
  let saveresult = cb.rest.invokeFunction("ST.otherOutRecord.saveotherin", { body: body, othinstatus: "OTHININSERT", othindata: applydata }, function (err, res) {}, viewModel, { async: false });
...
function updateOthout(othindata) {
  ...
  let result = cb.rest.invokeFunction("AT18E3E0443F380007.afterapi.updateOthOut", { billdata }, function (err, res) {});
}
...
viewModel.get("button42ra") &&
  viewModel.get("button42ra").on("click", function (data) {
    //审核--单击
    setTimeout(function () {
      let othindata = viewModel.getAllData();
      let cancel = othindata.cancel;
...
viewModel.get("button54zc") &&
  viewModel.get("button54zc").on("click", function (data) {
    //弃审--单击
    setTimeout(function () {
      let othindata = viewModel.getAllData();
      let verifystate = othindata.verifystate;
      ...
      if ("2" == verifystate && "1" == cancel) {
        ...
        if (!isEmpty(othinmainid)) {
          ...
          if (queryres.hasOwnProperty("result") && queryres.result.hasOwnProperty("auditres") && queryres.result.auditres.length > 0) {
            ...
            let result = cb.rest.invokeFunction("AT18E3E0443F380007.afterapi.aftecancel", { othindata: othindata, rewriteorcancel: "UNADUIT" }, function (err, res) {}, viewModel, { async: false });
            viewModel.execute("refresh");
          }
        }
      }
