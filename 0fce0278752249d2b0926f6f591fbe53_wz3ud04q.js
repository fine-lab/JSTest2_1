run = function (event) {
        ...
        (data) => {
          ...
          if (data.length > 0) {
            ...
            for (let j = 0; j < data.length; j++) {
              if (data[j] != null) {
                let dateIndex = dateId.indexOf(data[j].id);
                if (data[j].id == releasePlan && dateIndex == -1) {
                  let releaseItems = data[j].release_items_childList;
  ...
  function selectReleasePlanMaterial(orgId, materialId, skuId, businessType) {
    return new Promise((resolve) => {
      let invokeFunction1 = function (id, data, callback, options) {
        var proxy = cb.rest.DynamicProxy.create({
          doProxy: {
