...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    var sql = "select * from mka.activity.Activity where id = '" + id + "'";
    var Activity = ObjectStore.queryByYonQL(sql, "yycrm");
    if (Activity[0] != null) {
      var synClaimMaterial = Activity[0].synClaimMaterial;
    }
    if (synClaimMaterial == true) {
      //根据营销活动id查营销物料申领表的数据
      var sql1 = "select * from dsfa.terminalassets.TerminalAssets where activityId = '" + id + "'";
      var TerminalAssets = ObjectStore.queryByYonQL(sql1, "yycrm");
      throw new Error(JSON.stringify(TerminalAssets));
      if (TerminalAssets[0] != null) {
        var activityId = TerminalAssets[0].id;
      } else {
        throw new Error("查询不到营销物料申领单");
      }
      //获取营销物料子表信息
      var sql2 = "select * from dsfa.terminalassets.TerminalAssetsDetail where terminalassetsId = '" + activityId + "'";
      var TerminalAssetsDetail = ObjectStore.queryByYonQL(sql2, "yycrm");
