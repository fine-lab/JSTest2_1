...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    if (param.data && param.data.length > 0) {
      ...
      param.data[0].set("otheroutbusinesstype", "110000000000013");
    }
    return {};
  }
}
...
function setBrokenQty(orgId, productId, stockUnitId, param) {
  ...
  //物料检验破坏数量
  let querySQL = "select define.define3 from pc.product.Product where id = " + productId;
  let res = ObjectStore.queryByYonQL(querySQL, "productcenter");
  if (!res || res.length === 0) {
    param.data[0].set("brokenQty", "0");
    param.data[0].set("brokenSubQty", "0");
  } else {
    param.data[0].set("brokenQty", res[0].define_define3 + "");
    param.data[0].set("brokenSubQty", (Number(res[0].define_define3) / Number(param.data[0].invexchrate)).toFixed(3) + "");
  }
}
//补充检品来源
