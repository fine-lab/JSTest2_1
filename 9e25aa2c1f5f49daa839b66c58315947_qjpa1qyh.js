    ...
    if (bvolist !== null && bvolist !== undefined) {
      ...
      for (let key in map) {
        ...
        var bodyheadquery = {
          ...
          pageSize: "10",
          simple: {
            "materialId.productId": [materialnew]
          }
        };
        let apiResponsequery = postman("post", "https://www.example.com/" + token, JSON.stringify(header), JSON.stringify(bodyheadquery));
        var apiResponsequeryjson = JSON.parse(apiResponsequery);
        var queryCodequery = apiResponsequeryjson.code;
        if (queryCodequery !== "200") {
          throw new Error("错误" + apiResponsequeryjson.message + JSON.stringify(bodyheadquery));
        } else {
          throw new Error("111");
          var querydata = apiResponsequeryjson.data.recordList;
          if (querydata.length > 0) {
            for (var x = 0; x < querydata.length; x++) {
              let queryid = querydata[x].id;
              let headerqxsh = {
                "Content-Type": hmd_contenttype
              };
              let bodyqxsh = {
              ...
              if (qxshcode !== "200") {
                throw new Error("错误" + qxshjson.message + JSON.stringify(bodyqxsh));
              } else {
                //再删除
                ...
                if (sccode !== "200") {
                  throw new Error("错误" + scjson.message + JSON.stringify(bodyqxsh));
                }
              }
            }
          }
        ...
        for (let j = 0; j < blist.length; j++) {
          var bvo1 = blist[j];
          var jsonb = {
            productId: bvo1.peijian,
            scrap: 0,
            numeratorQuantity: bvo1.shuliang,
        ...
        bodyhead.data.bomComponent = jsonbs;
        var jsonjson = JSON.stringify(bodyhead);
        let apiResponse1 = postman("post", "https://www.example.com/" + token, JSON.stringify(header), JSON.stringify(bodyhead));
        ...
        if (queryCode1 !== "200") {
          ...
          var boomdata = apiResponse1json.data.infos[0].data;
          ...
          if (idid !== undefined) {
            ...
            var jsonjsonboom = JSON.stringify(bodyheadboom);
            let apiResponseboom = postman("post", "https://www.example.com/" + token, JSON.stringify(header), JSON.stringify(bodyheadboom));
            var apiResponse1jsonboom = JSON.parse(apiResponseboom);
            var queryCodeboom = apiResponse1jsonboom.code;
            if (queryCodeboom !== "200") {
              throw new Error("错误" + idid + apiResponse1jsonboom.message + JSON.stringify(bodyheadboom));
            }
          }
        }
      }
