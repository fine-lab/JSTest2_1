function qilibcPrintInitUI(event) {
  ...
  viewModel.on("afterLoadData", function () {
    if (cb.utils.isEmpty(perData) || cb.utils.isEmpty(perData.billno)) {
      ...
      for (var i = 0; i < alldata.length; i++) {
        ...
        if (rowdata.productClass == undefined) {
        ...
        barcodedata.def5 = rowdata.isSerialNoManage;
  ...
  mainGirdModel.on("afterSetDataSource", () => {
    ...
    rows.forEach((data, idn) => {
      ...
      if (data.def5) {
        mainGirdModel.setCellState(idn, "def7", "disabled", false);
      } else {
        mainGirdModel.setCellState(idn, "def7", "disabled", true);
      }
  ...
  viewModel.get("button18yd").on("click", function (data) {
    ...
    proxy.queryData(param, function (err, result) {
      ...
      try {
        //等待框
        var tenantId = cb.rest.AppContext.tenant.tenantId;
        var baseUrl = cb.utils.getServiceUrl();
        var queryUrl = baseUrl + "/files/scmbc/barprint/exportPassword?domainKey=qilibcscm";
        var sendData = { rows: rows, tenantId: tenantId };
        const { common } = viewModel.biz.action();
        const endDown = common.createDownloadForm({ url: queryUrl, params: sendData });
      } finally {
      }
    });
  });
