viewModel.on("customInit", function (data) {
  ...
  viewModel.on("afterSave", function (args) {
    console.log("args", JSON.stringify(args.res));
    ...
    let data = {
      ...
      _status: "Insert",
      "headFreeItem!define5": billdata.id, //主表ID
      "headFreeItem!define6": billdata.code, //主表编码
      "headFreeItem!define1": billdata.saleType, //销售类型
      "headFreeItem!define4": billdata.paytype, //付款方式
      "headFreeItem!define3": billdata.salestaff, //员工（业务人员）
      retailAgentName: billdata.retailAgentName, //散户名称
      "headFreeItem!define11": billdata.accelement, //会计要素
      "headFreeItem!define12": billdata.retailPayMode //支付类型
    };
    data.orderDetails = [];
    ...
    for (let i = 0; i < GxyRetailDetailList.length; i++) {
      ...
      let detail = {
        ...
        _status: "Insert",
        "bodyFreeItem!define5": billdata.id, //主表ID
        "bodyFreeItem!define6": billdata.code, //主表编码
        "bodyFreeItem!define7": details.id, //子表id
        stockId: billdata.stock, //仓库
        "bodyFreeItem!define1": billdata.saleType //销售类型
      };
