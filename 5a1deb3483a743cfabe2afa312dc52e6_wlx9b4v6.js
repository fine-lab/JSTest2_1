...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    for (var i = request.gsmcs.length - 1; i >= 0; i--) {
      ...
      let jfsy; //积分剩余
      let jfljzh; //积分累计+初始化
      let tyhtsql = "select sum(bencihuodejifen),sum(jifen) from GT56762AT44.GT56762AT44.QTshoukuan where gongsimingchen='" + qykid + "'  "; //  通用合同收款
      ...
      let zzhtsql = "select sum(bencihuodejifen),sum(jifen) from GT57700AT57.GT57700AT57.QTZZSK where qiyemingchen='" + request.gsmcs[i] + "'  and code not in('" + request.docdate + "')"; //  资质合同收款
