run = function (event) {
  ...
  viewModel.on("beforeBatchpush", function (data) {
    ...
    if (data.args.cCaption == "退货复核") {
      ...
      return returnPromise;
    } else if (data.args.cCaption == "采购退货") {
      for (let i = 0; i < selectData.length; i++) {
        let orgId = selectData[i].org;
        let qty = selectData[i].qty;
        let billNo = selectData[i].code;
        promises.push(validateReturnBill(orgId, qty, billNo).then(handerMessage));
      }
  ...
  function validateReturnBill(orgid, qty, billno) {
    return new Promise(function (resolve, reject) {
      invokeFunction1(
        "GT22176AT10.publicFunction.getGBilltype",
        {
          orgId: orgid,
          type: "st_purchaseorder"
        },
        function (err, res) {
          let message = "";
          if (err) {
            message = "单据编号为" + billno + "的单据查询交易类型报错：" + err.message;
          } else {
            let types = res.types;
            let isTran = types.indexOf(bustypeid);
            if (qty < 0 && isTran > -1) {
              message = "单据编号为" + billno + "的GSP红字采购订单不支持红字采购到货";
            }
          }
          resolve(message);
        },
