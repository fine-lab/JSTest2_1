...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    var mainSql = "select distinct organizationId from AT15F164F008080007.AT15F164F008080007.DetectOrder where dr = 0 and pingzhenghao = '" + OddNumbers + "' and importData = '" + date + "'";
    ...
    for (var i = 0; i < mainRes.length; i++) {
      ...
      var sql =
        "select distinct displayName,accBook,periodUnion from egl.voucher.VoucherBO where dr = 0 and accBook = '" +
        zhbcode +
        "' and displayName = '记-" +
        OddNumbers +
      ...
      if (resa.length != 0) {
        throw new Error("【" + dates + "】成本凭证号在此期间已经存在请删除后再进行清除凭证号!");
      }
    }
    //获取样本编号;
    var ponseSql = "select sampleCode from AT15F164F008080007.AT15F164F008080007.DetectOrder where dr = 0 and pingzhenghao = '" + OddNumbers + "' and importData = '" + date + "'";
