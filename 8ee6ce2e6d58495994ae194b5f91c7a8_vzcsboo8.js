...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    function customerdoc_interface(act, body, activity) {
      var list_body_strResponse = postman("post", "https://www.example.com/" + activity + "?access_token=" + act + "&", null, JSON.stringify(body));
      var Response = JSON.parse(list_body_strResponse);
      return Response;
    }
    var ay = "9a2259ec481c43e79ce6346a306ef546";
    var ac = "e801436482d947b596ba32672de827be";
    var act = getToken(ay, ac);
    var now_dlt_body = param.data;
    var list_body = {};
    var dlt_body = {};
    dlt_body.data = [];
    list_body.pageSize = "10000";
    list_body.pageIndex = "1";
    list_body.schemeName = "customerdoc_list";
    list_body.isDefault = "true";
    list_body.custdocdefid = "youridHere";
    list_body.externalData = {};
    list_body.externalData.serviceCode = "GZTBDM041";
    list_body.billnum = "customerdoc_list";
    var list = customerdoc_interface(act, list_body, "list");
    if ((list.code = "200")) {
      if (list.data.recordCount == 0) {
        throw new Error("fail no record ！");
        return "fail no record";
      } else if (list.data.recordCount >= 1) {
        var recordList = list.data.recordList;
      }
    }
    if (now_dlt_body != undefined) {
      for (var now_dlt of now_dlt_body) {
        var code = now_dlt.code;
        var id = get_id(recordList, code);
        if (id == undefined) {
          console.log(code + "is not exsit");
          break;
        } else {
          var id_body = { id: id };
          dlt_body.data.push(id_body);
        }
      }
    }
    if (dlt_body.data.length == 0) {
      return "yonbuidl delete conplete,system no record ";
    } else {
      var deleteResponse = customerdoc_interface(act, dlt_body, "delete");
