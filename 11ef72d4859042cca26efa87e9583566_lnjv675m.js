viewModel.on("customInit", function (data) {
  ...
  viewModel.on("afterSave", function (args) {
    ...
    let data = {
      "orderPrices!id": billdata.OrderID,
      "orderPrices!orderId": billdata.OrderID,
      resubmitCheckKey: guid, //幂等性随机数
      salesOrgId: billdata.org_id, //销售组织
      transactionTypeId: billdata.OrdertransType, //交易类型
      vouchdate: billdata.billdate, //单据日期
      code: billdata.code, //单据编号
      code: billdata.OrderCode,
      ...
      retailAgentName: billdata.retailAgentName //散户名称//散户名称
    };
    if (billdata.OrderID == undefined) {
      //说明单据是没有同步到销售订单的，此次保存即为新增态
      data._status = "Insert";
      data["orderPrices!totalNatTax"] = billdata.totalOriTax;
      data["headFreeItem!define5"] = billdata.id; //主表ID
      data["headFreeItem!define6"] = billdata.code; //主表编码
      data["headFreeItem!define1"] = billdata.saleType; //销售类型
      data["headFreeItem!define4"] = billdata.paytype; //付款方式
      data["headFreeItem!define3"] = billdata.salestaff; //员工（业务人员）
      data["headFreeItem!define11"] = "65e9b261-3b76-11eb-ac7b-7cd30abeaf4c";
      data["headFreeItem!define12"] = billdata.retailPayMode; //支付类型
    } else {
      //更新态
      data.id = billdata.OrderID;
      data._status = "Uptate";
    }
    data.orderDetails = [];
    ...
    for (let i = 0; i < GxyRetailDetailList.length; i++) {
      ...
      let detail = {
        id: details.OrderDetailID,
        "orderDetailPrices!id": details.OrderDetailID,
        "orderDetailPrices!orderDetailId": details.OrderDetailID,
        ...
        orderProductType: details.orderProductType, //商品售卖类型
