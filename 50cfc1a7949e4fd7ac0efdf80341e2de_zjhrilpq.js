...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    var body = { count: 10, productId: "yourIdHere" };
    let body1 = JSON.stringify(body);
    let head = JSON.stringify(header);
    let uri = hmd_base_path.concat(hmd_req_resources_path);
    //请求数据
    let apiResponse = postman("post", uri, head, body1);
