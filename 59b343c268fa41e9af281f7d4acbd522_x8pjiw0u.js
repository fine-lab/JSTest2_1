...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (picihao == null || picihao.length == 0) {
      sql = "select * from ia.account.IADetailLedgerVO where costreg = '" + chengbenyuId + "' and material = '" + wuliaoId + "' order by createTime";
    } else {
      sql = "select * from ia.account.IADetailLedgerVO where costreg = '" + chengbenyuId + "' and material = '" + wuliaoId + "' and batchcode = '" + picihao + "' order by createTime";
    }
