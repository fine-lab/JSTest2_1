viewModel.on("customInit", function (data) {
  ...
  function addPartners() {
    //查询默认伙伴
    cb.rest.invokeFunction("GT5258AT16.yswbzyytd.queryDefPart", { platform: "1" }, function (err, res) {
      debugger;
      console.log(res);
      var gridModel = viewModel.get("apply_partner_detailList");
  ...
  //终止
  viewModel.get("button41rf") &&
    viewModel.get("button41rf").on("click", function (data) {
      // 终止--单击
  ...
  //激活
  viewModel.get("button55eb") &&
    viewModel.get("button55eb").on("click", function (data) {
      // 终止--单击
      var id = viewModel.get("id").getValue();
      cb.utils.confirm(
        "确认继续激活？",
  ...
  viewModel.on("afterSave", function (args) {
    var id = viewModel.get("id").getValue();
    let res = cb.rest.invokeFunction("GT5258AT16.yswbzyytd.addPrlPartner", { id, platform: "1" }, function (err, res) {}, viewModel, { async: false });
    console.log(JSON.stringify(res));
    viewModel.execute("refresh");
  });
});
viewModel.get("button69rc") &&
  viewModel.get("button69rc").on("click", function (data) {
    // 客开伙伴--单击
    var currentState = viewModel.getParams().mode;
    if (currentState == "add") {
      var gridModel = viewModel.get("apply_partner_detailList");
      gridModel.clear();
      cb.rest.invokeFunction("GT5258AT16.yswbzyytd.queryKekaiPart", { platform: "1" }, function (err, res) {
        if (res && res.links && res.links.length > 0) {
          var links = res.links;
          for (var i = 0; i < links.length; i++) {
            var kkp = links[i];
            console.log(JSON.stringify(kkp));
            var partner = {
              responseStatus: "1",
              hasDefaultInit: true,
              _tableDisplayOutlineAll: false,
              partnerDoc: kkp.id,
              partnerDoc_partner_name: kkp.partnerName,
              serviceAreaName: kkp.serviceAreaName,
              projectNum: kkp.projectNum,
              projectSatis: kkp.projectSatis,
              introduce: kkp.introduce,
              partner: kkp.partner,
              partner_name: kkp.partnerName,
              partnerName: kkp.partnerName,
              isPublic: "N"
            };
            gridModel.appendRow(partner);
          }
        }
      });
    } else {
      cb.utils.alert("非新增态状态下该按钮无效！");
    }
  });