...
let productInfoList = viewModel.getGridModel("sy01_udi_product_infov3List"); //关联的商品列表
let productConfigure2List = viewModel.getGridModel("sy01_udi_product_configurev3List"); //关联的商品列表 子表配置包装信息
...
viewModel.get("button32ci") &&
  viewModel.get("button32ci").on("click", function (data) {
    ...
    // 清空下表数据
    productConfigure2List.deleteAllRows();
    // 加载包装标识--单击
    let productSql = "select * from I0P_UDI.I0P_UDI.sy01_udi_product_list_bzbsxxv3 where sy01_udi_product_list_id = '" + proId + "'";
...
productConfigure2List
  ...
  .on("afterValueChange", function (data) {
    console.log(data);
    var gridmodel2 = viewModel.get("sy01_udi_create_info3List");
    // 清空下表数据
    gridmodel2.clear();
    let sqlConfig = "select * from I0P_UDI.I0P_UDI.sy01_udi_create_config_sonv3 where sy01_udi_create_config_id = '" + data.value.id + "'";
    cb.rest.invokeFunction(
      "I0P_UDI.publicFunction.shareApi",
      {
        //传入参数 sqlType：类型
...
viewModel.get("productUdi_zxxsdycpbs") &&
  viewModel.get("productUdi_zxxsdycpbs").on("afterValueChange", function (data) {
    ...
    let sqlInfo = "select id from I0P_UDI.I0P_UDI.sy01_udi_relation_productv3 where productUdi ='" + zxcpid + "'";
    cb.rest.invokeFunction(
      "I0P_UDI.publicFunction.shareApi",
      {
        sqlType: "check",
      ...
      function (err, res) {
        if (err != null) {
          ...
          if (resultData.length > 0) {
            cb.utils.alert("当前产品标识已经存在关联!请勿重复添加!", "waring");
            viewModel.get("productUdi_zxxsdycpbs").setValue();
            viewModel.get("productUdi").setValue();
...
viewModel.get("btnAddRowsy01_udi_product_infov3") &&
  viewModel.get("btnAddRowsy01_udi_product_infov3").on("click", function (data) {
    let rows = productInfoList.getRows();
    if (rows.length > 1) {
      cb.utils.alert("一个产品标识只能关联一个商品！");
      productInfoList.deleteRows(1);
    }
  });