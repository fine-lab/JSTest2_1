...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    var orderDetails = data.st_transferapplylist;
    //声明一个数组，存放后台的字段
    var datalos;
    var temp = new Array();
        ...
        if (totalsummary.length > 0) {
          ...
          for (var i = 0; i < b; i++) {
            if (totalsummary[i] == dataod.headDefine.define3_name) {
              ...
              //在json对象中添加键值对数据
              //总数量
              materialClassifySumJSON.num = dataod.subQty / 1000;
              materialClassifySumJSON.Original_number = dataod.code;
              materialClassifySumJSON.commodity = dataod.outwarehouse;
    ...
    if (obj.code == 999) {
      throw new Error("id为" + temp + "的【调拨单单】插入了【运费结算单】实体失败" + ",失败原因为:" + obj.message);
    }
    if (obj.code == 310083) {
      throw new Error("id为" + temp + "的【调拨单单】插入了【运费结算单】实体失败" + ",失败原因为:" + obj.message);
    } else {
      var object = { id: "youridHere", name: "value" };
      var rescfd = ObjectStore.updateById("st.transferapply.TransferApply", object, "");
