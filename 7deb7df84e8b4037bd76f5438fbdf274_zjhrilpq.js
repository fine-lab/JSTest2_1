...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    var hmd_req_resources_path = "/homedo-yonsuite-service/updateOrderSaasRecord";
    //注意需要大写
    ...
    ybresult.forEach((data) => {
      if (data.hmdorderid != undefined) {
        let body = {
          orderId: data.hmdorderid,
          status: 2
        };
        //请求数据
        let apiResponse = postman(hmd_req_type, hmd_base_path.concat(hmd_req_resources_path), JSON.stringify(header), JSON.stringify(body));
        var object = new Object();
        object.isorderupdate = "1";
        object.id = data.id;
        let res = ObjectStore.updateById("GT1281AT3.GT1281AT3.ysordermng", object);
      }
    });
    //数据转换-----------------------end
