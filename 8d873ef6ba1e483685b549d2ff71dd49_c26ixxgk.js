...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      let hRefName = null;
      //查询销售发票自定义项
      let queryHdef = "select * from voucher.invoice.SaleInvoiceFreeItem where id='" + ids[i] + "'";
      let hdefRes = ObjectStore.queryByYonQL(queryHdef, "udinghuo");
      if (hdefRes[0] != null && hdefRes[0].define2 != null) {
        //根据自定义项主键查询自定义档案
        let queryhRefSql = "select name from bd.basedocdef.CustomerDocVO where id=" + hdefRes[0].define2;
        let hRefRes = ObjectStore.queryByYonQL(queryhRefSql, "ucfbasedoc");
        hRefName = hRefRes[0].name;
      }
      if (hdefRes[0] != null && hdefRes[0].define4 != null) {
        //根据自定义项主键查询客开的税票号码
        let queryTaxSql = "select fapiaoshuihao from AT1590F01809B00007.AT1590F01809B00007.invoiceTaxNo where id=" + hdefRes[0].define4;
        let taxRes = ObjectStore.queryByYonQL(queryTaxSql, "developplatform");
        let oldTax = taxRes[0].fapiaoshuihao; //需将税号中的.和-都替换掉
        if (oldTax != null) {
      ...
      if (bodyRes.length > 0) {
        for (var j = 0; j < bodyRes.length; j++) {
          //查询物料自定义项
          let queryBdef = "select * from pc.product.ProductFreeDefine where id=" + bodyRes[j].productId;
          var bdefRes = ObjectStore.queryByYonQL(queryBdef, "udinghuo");
          let TARIF_PPNBMValue = 0;
          if (bdefRes[0] != null && bdefRes[0].define1 != null) {
            //查询物料档案的明贵品级别
            //根据自定义项主键查询自定义档案
            let querybRefSql = "select name from bd.basedocdef.CustomerDocVO where id=" + bdefRes[0].define1;
            let bRefRes = ObjectStore.queryByYonQL(querybRefSql, "ucfbasedoc");
            TARIF_PPNBMValue = bRefRes.length == 0 ? 0 : bRefRes[0].name;
          }
      ...
      //查询销售发票单价编码
      let queryCodeSql = "select code,vouchdate,invAgentId,invAgentName,invAgentTaxNo,invAgentAdress,oriMoney from voucher.invoice.SaleInvoice where id=" + ids[i];
      let codeRes = ObjectStore.queryByYonQL(queryCodeSql, "udinghuo");
      let codeValue = codeRes[0].code;
      let vouchdateValue = codeRes[0].vouchdate.substr(0, 10).split("-");
      ...
      fkbody.push(codeRes[0].invAgentTaxNo); //开票客户纳税人识别号
      fkbody.push(codeRes[0].invAgentName); //开票客户名称
      fkbody.push(codeRes[0].invAgentAdress); //开票客户纳税人地址
      fkbody.push(Math.floor(codeRes[0].oriMoney)); //不含税金额--舍位只要整数
      fkbody.push(Math.floor(allLJ)); //税额(廉价)--舍位只要整数
      fkbody.push(Math.floor(allMG)); //税额(名贵品)--舍位只要整数
      if (hdefRes[0] != null && hdefRes[0].define3 != null) {
        fkbody.push(hdefRes[0].define3); //名贵品解释号
      } else {
      ...
      if (hdefRes[0] != null && hdefRes[0].define5 != null) {
        fkbody.push(hdefRes[0].define5); //新增加文本字段
      } else {
