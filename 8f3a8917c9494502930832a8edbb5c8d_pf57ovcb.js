...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    sql = "select distinct orderCode,orderId from usp.signconfirmation.SignConfirmations where mainid in (" + param1.join(",") + ")";
    let resDismiss = ObjectStore.queryByYonQL(sql, "uscmpub");
    for (let i = 0; i < resDismiss.length; i++) {
      sql =
        "select orderId.code,orderId.headFreeItem.define1,orderId.headFreeItem.define5,sum(subQty),sum(oriSum) from voucher.order.OrderDetail where orderId.id = '" +
        resDismiss[i].orderId +
        "' group by orderId.code,orderId.headFreeItem.define5"; //
      let resOrder = ObjectStore.queryByYonQL(sql, "udinghuo");
      if (resOrder.length > 0 && resOrder[0].orderId_headFreeItem_define5 != undefined && resOrder[0].orderId_headFreeItem_define5 == "已生成") {
        ...
        if (resvid.length > 0) {
          ...
          let apiResponse = openLinker("POST", url, "SCCS", JSON.stringify(body));
        }
      }
      //自定义项初始值
