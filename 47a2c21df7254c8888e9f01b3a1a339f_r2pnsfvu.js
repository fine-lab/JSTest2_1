...
class QueryPurchaseMonthlyHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (monthlyPlans && monthlyPlans.length > 0) {
      ...
      let bills = ObjectStore.selectBatchIds("GT7139AT4.GT7139AT4.sy_factoryorders", { ids: factoryBills });
      let billstatusMap = new Map();
  ...
  getAnnualAdditional(monthlyPlans, annualPlans) {
    ...
    for (let plan of annualPlans) {
      ...
      if (!monthly) {
        let newMonthlyPlan = this.clearBillInfo(plan);
        newMonthlyPlan.planNum = plan.confirmNum;
        newMonthlyPlan.internalPrice = plan.confirmPrice;
        needsupplement.push(newMonthlyPlan);
      } else {
        monthly.planNum = plan.confirmNum;
        monthly.internalPrice = plan.confirmPrice;
        monthly.purchaseNum && (monthly.purchaseAmount = monthly.purchaseNum * plan.confirmPrice);
        monthly.nextMonthNum && (monthly.nextMonthAmount = monthly.nextMonthNum * plan.confirmPrice);
        monthly.manxNum && (monthly.manxAmount = monthly.manxNum * plan.confirmPrice);
      }
    }
  ...
  initMonthlyPlans(annualPlans, monthly) {
    ...
    for (let plan of annualPlans) {
      ...
      plan.planNum = plan.confirmNum;
      plan.purchaseNum && (plan.purchaseAmount = plan.purchaseNum * plan.confirmPrice);
      plan.nextMonthNum && (plan.nextMonthAmount = plan.nextMonthNum * plan.confirmPrice);
      plan.manxNum && (plan.manxAmount = plan.manxNum * plan.confirmPrice);
      this.clearBillInfo(plan);
    }
    return annualPlans;
  }
