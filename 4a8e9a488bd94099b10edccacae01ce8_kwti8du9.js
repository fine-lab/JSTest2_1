...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    if (requestData.verifystate == 0) {
      ...
      if (requestData.stockorg == "2522102344422656") {
        //依安工厂
        requestData.WMSwarehouseId = "yourIdHere";
        requestData.WMScustomerId = "yourIdHere";
      } else if (requestData.stockorg == "2390178757465088") {
        //克东
        requestData.WMSwarehouseId = "yourIdHere";
        requestData.WMScustomerId = "yourIdHere";
      } else if (requestData.stockorg == "2369205391741184") {
        //电商
        if (requestData.warehouse == "1533684855346298884") {
          requestData.WMSwarehouseId = "KSDS";
        }
        requestData.WMScustomerId = "001";
      }
      ...
      for (var i = 0; i < bRes.length; i++) {
        ...
        //查询批次号
        let queryBatchnoSql =
          "select producedate,invaliddate,product,productsku,batchno from st.batchno.Batchno where product='" +
          bres.material +
          "' and productsku='" +
          bres.materialsku +
          "' and batchno='" +
        ...
        if (bres.quantity > 0) {
          ...
          let bodyData1 = {
            ...
            outStockStatusDoc: "02", //来源库存状态id或code
            org: requestData.stockorg, //组织id或code
            inStockStatusDoc: "01", //目的库存状态id或code ，放行
        ...
        if (bres.disquantity > 0) {
          ...
          let bodyData2 = {
            ...
            outStockStatusDoc: "02", //来源库存状态id或code
            org: requestData.stockorg, //组织id或code
            inStockStatusDoc: "04", //目的库存状态id或code ，禁用
            stockUnitId: bres.assistunit, //库存单位id或code
