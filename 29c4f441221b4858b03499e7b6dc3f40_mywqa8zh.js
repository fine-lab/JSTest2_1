...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    //如果look_num没定义就定义赋值为0
    if (look_num == undefined) {
      look_num = 0;
    }
    //无论是否已经查看都需要改次数   -----------------更新异常记录表的查看次数
    var object = { id: id, look_num: look_num + 1 };
    var res = ObjectStore.updateById("AT161E17CC09C80003.AT161E17CC09C80003.abnormalevent", object);
    //获取当前用户的员工信息-------------------------------------------------------------------
    ...
    var belongArea = "select '' as id ,id as areaid from AT161E17CC09C80003.AT161E17CC09C80003.Arealhg where StaffNew='" + psnid + "'";
    var belongR = ObjectStore.queryByYonQL(belongArea);
    if (undefined == belongR || belongR.length == 0) {
      var belongStore = "select id,area.id as areaid from AT161E17CC09C80003.AT161E17CC09C80003.stores where StaffNew='" + psnid + "'";
    ...
    var looklogresult = ObjectStore.insert("AT161E17CC09C80003.AT161E17CC09C80003.lookLog_lhg", looklog, "203bcc52");
    return { result: true };
  }
}
