...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    orderData.forEach((item) => {
      ...
      var queryProduct = postman("post", "https://www.example.com/" + token, null, JSON.stringify({ skuId: skuIdParams }));
      let productId = JSON.parse(queryProduct).data[0] && JSON.parse(queryProduct).data[0].productId;
      let param = {
        pageIndex: 1,
        pageSize: 10,
        isSum: false,
      ...
      var queryOrder = postman("post", "https://www.example.com/" + token, null, JSON.stringify(param));
      param.pageSize = JSON.parse(queryOrder).data.recordCount;
