...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    //校验code是否已经存在
    var isCodeExist = ObjectStore.queryByYonQL('select code, id from GT66350AT5.GT66350AT5.payment where dr=0 and orderno="' + code + '"');
    if (isCodeExist.length > 0) {
      throw new Error("该单据已经生成付款申请单");
    }
    //获取应付事项详情
    let func1 = extrequire("GT66350AT5.fioap.fiaopdetails");
    ...
    //查询付款申请进行抵消
    var chSum = ObjectStore.queryByYonQL('select oriSum, id,isSync from GT66350AT5.GT66350AT5.prepayment_ljc	 where dr=0 and  orderno="' + orderno + '"');
    if (chSum.length > 0) {
      for (let j in chSum) {
        if (chSum[j].isSync == "2") {
    ...
    var res = ObjectStore.insert("GT66350AT5.GT66350AT5.payment", insertData, "48b31ea5");
    return { res };
  }
}
