    ...
    viewModel.on("modeChange", function (data) {
      if (data === "add" || data == "edit") {
        ...
        for (let i = 0; i < rows.length; i++) {
          gjrkys_getCustomerInfo(rows[i].product, org_id).then((materialInfo) => {
            gridModelInfo.setCellValue(i, "expireDateNo", materialInfo.detail.expireDateNo);
            gridModelInfo.setCellValue(i, "expireDateUnit", materialInfo.detail.expireDateUnit);
          });
        }
      }
    });
