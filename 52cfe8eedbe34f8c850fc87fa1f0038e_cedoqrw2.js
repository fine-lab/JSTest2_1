...
viewModel.on("afterLoadData", function (data) {
  ...
  if (!define2 && supplierId) {
    cb.rest.invokeFunctionS("65f598b230874cc08fccbd97c3b6004a", { supplierId }, function (err, res) {
      if (res && res.result) {
        ...
        if (openaccountbank) {
          cb.rest.invokeFunctionS("9d01c4e5055344a1a07b86418a2c10a8", { openaccountbank }, function (err, res) {
            if (res && res.result) {
  ...
  //其他单据生成合同 携带信息填充
  var reqDatas = viewModel.originalParams.reqDatas;
  if (reqDatas && reqDatas.length > 0) {
    reqDatas.forEach((item) => {
      let materialData = {
        ...
        srcBillType: "requirement",
        firstBillcode: item.code,
        firstBillRowno: item.sourcerowno
      };
      viewModel.getGridModel().appendRow(materialData);
    });
    viewModel.originalParams.reqDatas = {};
