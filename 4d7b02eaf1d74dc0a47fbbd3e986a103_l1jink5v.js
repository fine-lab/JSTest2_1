...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    let fdOrderSource = mainObj.fdOrderSource;
    let verifystate = mainObj.verifystate;
    if (fdOrderSource === "经销商" && verifystate != 4) {
      {
      }
    }
    {
      let detList = ObjectStore.queryByYonQL('select *,MProductTag.code as MProductTag_code from GT4691AT1.GT4691AT1.MFrontSaleOrderDet  where MFrontSaleOrderDetFk  ="' + mainObj.id + '"');
      //客户、法律实体、涉及物料
      //取出返利金额
      {
        let message;
