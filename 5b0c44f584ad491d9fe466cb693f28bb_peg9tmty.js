  ...
  viewModel.get("button48yh").on("click", function (data) {
    ...
    if (selectRows.length == 0) {
      ...
      for (var i = 0; i < selectRows.length; i++) {
        ...
        var productName_product_coding = selectOne.productName_product_coding;
        param = {
          code: code,
          number: number,
          sku: productName_product_coding
        };
        // 调用查询本地数据库接口
        ...
        if (DBresult.error) {
          ...
          if (DBresult.result.hasOwnProperty("res") && DBresult.result.res.length > 0) {
            // 从本地数据库查询到数据
