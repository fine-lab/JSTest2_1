...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    materialNccInfo.forEach((self) => {
      ...
      if (self.tsflag === "true") {
        if (nowDateTime > new Date(self.tstime).getTime() - 28800000) {
          // 停售且停售时间小于当前时间则下架
      ...
      } else if (nowDateTime > new Date(self.tctime).getTime() - 28800000) {
    ...
    if (queryInventoryParam.data.length > 0) {
      var inventorys = extrequire("SCMSA.saleOrderRule.queryInventoryApi").execute(queryInventoryParam);
    ...
    if (shelvesProducts.length > 0) {
      let execuProducts = [];
      shelvesProducts.forEach((self, index) => {
        shelvesProduct(self);
      });
    }
    // 批量下架
    if (unshelvesProducts.length > 0) {
      let execuProducts = [];
      unshelvesProducts.forEach((self, index) => {
        unshelvesProduct(self);
      });
    }
    // 响应
    return {};
    ...
    function getMaterialInfoByNcc(params) {
      let res = postman("post", config.nccUrl + "/servlet/materialStatusQueryServlet", "", JSON.stringify(params));
      try {
        // 转为JSON对象
        res = JSON.parse(res);
        // 返回信息校验
