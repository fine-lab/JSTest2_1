...
viewModel.on("customInit", function (data) {
  ...
  cb.cache.set("learning_plan_isReplace", false);
  //返回前操作
  viewModel.on("beforeAbandon", function (a) {
    if (ts) {
      clearInterval(ts);
      ts = null;
    }
    ...
    // 清除缓存 设置可以数据替换
    cb.cache.clear("learning_plan_isReplace");
    cb.cache.set("learning_plan_isReplace", true);
  });
  // 学员端-课程学习2详情--页面初始化
  ...
  viewModel.on("afterLoadData", function () {
    ...
    cb.cache.set("learning_plan_ts_cache", ts);
  });
});
