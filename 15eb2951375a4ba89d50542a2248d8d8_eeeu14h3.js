...
var stocks_temp = [];
var wareList = [];
viewModel.get("button59ui") &&
  viewModel.get("button59ui").on("click", function (data) {
    // 最优投料--单击
    console.log("最优投料--单击-button59ui");
    //判断单据状态，非开立不允许执行
      ...
      cb.utils.confirm(
        ...
        function () {
          ...
          if (currentState == "edit") {
            //子件刷新按钮
            //生效，有可能出现异步现象
            var button59ui = viewModel.get("button59ui");
...
function queryCurrentStockBy(args) {
  if (access_token == "" || lastGetDate == undefined || lastGetDate.getTime() + 7200 * 1000 < new Date().getTime()) {
    var tokenjson = cb.rest.invokeFunction("PO.backDesignerFunction.GetToken", {}, function (err, res) {}, viewModel, { async: false });
  ...
  try {
    var json = cb.rest.invokeFunction("PO.backDesignerFunction.GetCurrStock", param, function (err, res) {}, viewModel, { async: false });
...
function queryWare(args) {
  ...
  var json = cb.rest.invokeFunction("PO.backDesignerFunction.GetWare", param, function (err, res) {}, viewModel, { async: false });
...
function queryWareByOrg(orgid) {
  ...
  console.log("查询仓库参数:" + JSON.stringify(param));
  try {
    var json = cb.rest.invokeFunction("PO.backDesignerFunction.GetWareList", param, function (err, res) {}, viewModel, { async: false });
    console.log("查询仓库结果:" + json.result.strResponse);
    return JSON.parse(json.result.strResponse);
  } catch (ex) {
    return null;
...
function judgeOptinal(rows) {
  ...
  for (var i = 0; i < materials.length; i++) {
    ...
    if (stockdata != null && material.groupIndex < stockdata.length) {
      ...
      if (material.warehouseId == row.warehouse) {
        if (material.skuId == row.productsku) {
          if (material.recipientQuantity > row.availableqty) {
            bOptinal = false;
          }
        } else {
          bOptinal = false;
    ...
    if (bFlag) {
      ...
      for (var j = 0; j < stockRows.length; j++) {
        ...
        if (stockRow.availableqty > recipientQuantity) {
          ...
          newRow.auxiliaryRecipientQuantity = recipientQuantity;
          newRow.bomAuxiliaryRecipientQty = recipientQuantity;
          recipientQuantity = 0;
        } else {
          newRow.recipientQuantity = stockRow.availableqty;
          newRow.auxiliaryRecipientQuantity = stockRow.availableqty;
