...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    //取费用收入单
    var fy = ObjectStore.queryByYonQL("select * from GT3217AT5.GT3217AT5.YDSRML where suoshuhuijiqijian='" + request.huijiqijianid + "' and dr=0 ");
    if (fy.length == 0) {
    ...
    fbfdfp.forEach((item, index) => {
      ...
      for (var fbfdfpi = 0; fbfdfpi < fbres.length; fbfdfpi++) {
        if (item.fenbu == fbres[fbfdfpi].fenbu && fbres[fbfdfpi].fylx == 2) {
