...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    var sql = "select id,jifen from GT56481AT32.GT56481AT32.QYK  where gongsimingchen='" + param.data[0].gongsimingchen_gongsimingchen + "' ";
    ...
    var jfsy; //积分剩余
    var jfljzh; //积分累计+初始化
    var tyhtsql = "select sum(bencihuodejifen),sum(jifen) from GT56762AT44.GT56762AT44.QTshoukuan where gongsimingchen='" + qykid + "' "; //  通用合同收款
    ...
    var zzhtsql = "select sum(bencihuodejifen),sum(jifen) from GT57700AT57.GT57700AT57.QTZZSK where qiyemingchen='" + param.data[0].gongsimingchen_gongsimingchen + "'"; //  资质合同收款
    ...
    var zztksql = "select sum(jifen) from GT57293AT50.GT57293AT50.AQTZZTK where gongsimingchen='" + qykid + "' and code<>'" + param.data[0].code + "' "; //晴天资质退款
    ...
    if (param.data[0].jifen != undefined && param.data[0].jifen != "") {
      zztkbcxh = zztkbcxh + Number(param.data[0].jifen); //积分消耗总数
    }
    var htgdsql = "select sum(bencixiaofeijifen) from GT57529AT55.GT57529AT55.FWSP where qiyemingchen='" + qykid + "'"; //合同工单（服务审批）
