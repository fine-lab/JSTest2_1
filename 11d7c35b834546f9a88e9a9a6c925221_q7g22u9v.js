...
class MyTrigger extends AbstractTrigger {
  execute(context, param) {
    ...
    if (!data.purInRecords) {
      return {};
    }
    if (typeof data.purInRecords[0] == "undefined" || data.purInRecords[0] === null) {
      throw new Error("表体行为空！");
    }
    let billNum = data.purInRecords[0].priceQty;
    // 蓝字单据拦截，红字单据直接放行
    if (billNum < 0) {
      return {};
    }
    ...
    //准备匹配参数
    var objectArr = [];
    let { purInRecords } = data;
    for (var i = 0; i <= purInRecords.length - 1; i++) {
      let { product, productsku, producedate } = purInRecords[i];
      //如果生产日期为空则不加入校验数组
    ...
    var errorMsg = "";
    if (resArray.length <= 0 || resArray === null) {
      //没有寻到，直接放行
      return {};
    } else {
      //数据整理
