...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    dataList.forEach((row) => {
      ...
      //费用承担组织编码
      let codeOrg = row._widget_1671178501898;
      //费用承担组织名称
      let nameOrg = row._widget_1668060959595;
      //费用承担组织 undertakeOrg
      let undertakeOrg = getOrg(serialNumber, codeOrg, nameOrg);
      //费用承担部门编码
      let assumeDepartmentCode = row._widget_1672017909741;
      let vfinacedept = getDepartment(serialNumber, assumeDepartmentCode);
      //报销人  pk_handlepsn
      let pk_handlepsn = row._widget_1668498698066.name;
      ...
      //费用项目编码
      let xmCode = getXmCode(serialNumber, row._widget_1671433493133);
      //币种
      ...
      //备注
      //费用类型
      let expenseType = row._widget_1671178313535;
      let expenseTypeId = getExpenseType(serialNumber, expenseType, access_token, 1);
      let expensebillDcs = {};
      ...
      //应付总额
      map.nshouldpaymny = row._widget_1667444104866;
      //本币应付总额
      map.nnatshouldpaymny = row._widget_1667444104866;
      //付款总额
      map.npaymentmny = row._widget_1667444104866;
      //本币付款总额
      map.nnatpaymentmny = row._widget_1667444104866;
      //不含税总额
      map.nexpensemny = row._widget_1666936807444;
      //本币不含税总额
      map.nnatexpensemny = row._widget_1666936807444;
      //累计付款额
      map.naccpaymny = row._widget_1667444104866;
      //累计付款额-本币
      map.nnataccpaymny = row._widget_1667444104866;
      ...
      reimburseDetail.forEach((item) => {
        ...
        let project = item._widget_1666936807433 == "是" ? item._widget_1666936807435 : item._widget_1671178501911;
        let htCode = getHtCode(serialNumber, project);
        let jdyStop = item._widget_1666936807432;
        let businessId = getExpenseType(serialNumber, jdyStop, access_token, 2);
        let expensebillBDcs = {};
      ...
      if (reimburseType == "冲借款") {
        ...
        //借款/预付单号
        let prepayCode = row._widget_1670815769204;
        ...
        //核销额（含未审核）
        jkhxArr.nacccavmny = row._widget_1670815769212;
        //核销额(含未审核)-本币
        jkhxArr.nnatacccavmny = row._widget_1670815769212;
        //还款额（含未审核）
        jkhxArr.naccreturnmny = 0;
        //还款额(含未审核)-本币
        jkhxArr.nnataccreturnmny = 0;
        //余额
        jkhxArr.ntotalcavmny = row._widget_1670815769214;
        //余额-本币
        jkhxArr.nnattotalcavmny = row._widget_1670815769214;
        //本次核销
        ...
        //借款/预提原因
        jkhxArr.vreason = row.widget_1670815769205;
        //借款核销
      ...
      //付款金额
      jfxxArr.nsummny = row._widget_1667444104866;
      //期望收款金额
      jfxxArr.nsettlesummny = row._widget_1667444104866;
      //期望收款金额-本币
      jfxxArr.nnatsettlesummny = row._widget_1667444104866;
      //收款银行类别
      ...
      //获取简道云银行账户编码
      let bankCode = row._widget_1676359067601;
      //获取简道云银行账户名称
      let bankName = row._widget_1676359067599;
      ...
      let saveResponse = apiman("POST", saveUrl, JSON.stringify(saveHeader), JSON.stringify(saveBody));
      ...
      if (saveResponse.code == 200) {
        ...
        if (updateResponse.code == null) {
          ...
          let auditApiResponse = apiman("post", auditUrl, JSON.stringify(saveHeader), JSON.stringify(auditBody));
        ...
        throw new Error("流水号:" + serialNumber + "，ys保存返回的错误信息：" + saveResponse.message + "，ys保存的入参如下：" + JSON.stringify(saveBody));
      }
    ...
    function getCustomer(serialNumber, customer) {
      if (customer != "" && customer != null) {
        ...
        if (res3.length == 0) {
          throw new Error("流水号:" + serialNumber + "根据简道云客户编码未查到客户ID,客户编码：" + customer);
        } else {
          return res3[0].id;
        }
      } else {
    ...
    function getExpenseType(serialNumber, expenseType, access_token, type) {
      if (expenseType != "" && expenseType != null) {
        ...
        if (expenseTypeInfo == null || expenseTypeInfo == "") {
          throw new Error(JSON.stringify(expenseTypeBody) + "流水号:" + serialNumber + "没有查到费用类型id或jdy商机id" + type);
        } else {
