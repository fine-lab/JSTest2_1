viewModel.get("button46pb") &&
  viewModel.get("button46pb").on("click", function (data) {
    // 批量发送快递--单击
    ...
    if (rows.length == 0) {
      ...
      if (arrayRes.error) {
        cb.utils.confirm("查询枚举档案【快递类型】配置信息异常：" + arrayRes.error.message);
        return false;
      }
      var array = arrayRes.result.apiArray;
      for (let idnumber of rowMap.keys()) {
        var billDataRes = cb.rest.invokeFunction("SCMSA.backDesignerFunction.queryFHDBillData", { idnumber: idnumber, array: array }, function (err, res) {}, viewModel, { async: false });
        ...
        if (billData.def5 != null) {
          ...
          if (billData.def4 == "顺丰") {
            var sfRes = cb.rest.invokeFunction("SCMSA.backDesignerFunction.sendDataToSF", { billData: billData }, function (err, res) {}, viewModel, { async: false });
            if (sfRes.error) {
              cb.utils.confirm(rowMap.get(idnumber) + "发送顺丰快递失败：" + sfRes.error.message);
              return false;
            }
          } else {
            var cnRes = cb.rest.invokeFunction("SCMSA.backDesignerFunction.sendDataToCn", { billData: billData }, function (err, res) {}, viewModel, { async: false });
            if (cnRes.error) {
...
viewModel.get("button62rg") &&
  viewModel.get("button62rg").on("click", function (data) {
    // 批量面单打印--单击
    ...
    socket.onopen = function (event) {
      ...
      //获取选中数据
      for (var i = 0; i < rows.length; i++) {
        var dataRes = cb.rest.invokeFunction("SCMSA.backDesignerFunction.queryDataById", { idnumber: rows[i].id }, function (err, res) {}, viewModel, { async: false });
        if (dataRes.error) {
...
viewModel.get("voucher_deliverylist") &&
  viewModel.get("voucher_deliverylist").on("beforeSetDataSource", function (data) {
    // 表格--设置数据源前
    const btjmx = viewModel.get("sumSwitch").getValue();
    //表体+明细
    if (btjmx == false) {
      viewModel.get("button46pb").setVisible(false); //批量发送快递
      viewModel.get("button62rg").setVisible(false); //
    } else {
      //表头
      viewModel.get("button46pb").setVisible(true); //批量发送快递
      viewModel.get("button62rg").setVisible(true); //批量打印快递
    }
  });