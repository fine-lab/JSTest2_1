...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (clientData.code == 200) {
      clientCode = clientData.data.merchantDefine.define1 != undefined ? clientData.data.merchantDefine.define1 : undefined;
      if (clientCode == undefined) {
        throw new Error("SAP客户编码未维护");
      }
    } else {
    ...
    var zdyx = xxddsj.headFreeItem != undefined ? xxddsj.headFreeItem : undefined;
    if (zdyx == undefined) {
      throw new Error("启润单号和开票状态，未在单据中维护");
    }
    //销售订单号
    var vbeln = xxddsj.headFreeItem.define1 != undefined ? xxddsj.headFreeItem.define1 : undefined;
    if (vbeln == undefined) {
      throw new Error("销售订单号，未在单据中维护");
    }
    //开票状态
    var ZBILSTATUS = xxddsj.headFreeItem.define2 != undefined ? xxddsj.headFreeItem.define2 : undefined;
