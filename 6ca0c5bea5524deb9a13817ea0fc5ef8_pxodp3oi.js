...
viewModel.on("beforeSearch", (args) => {
  let startDate = viewModel.getCache("FilterViewModel").get("yuefen").getFromModel().getValue();
  let endDate = viewModel.getCache("FilterViewModel").get("yuefen").getToModel().getValue();
  if (startDate) {
    if (!endDate) {
      endDate = getDate();
    }
