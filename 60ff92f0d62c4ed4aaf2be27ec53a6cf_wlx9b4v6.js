...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    var result = [];
    //王霞可以看全部
    if ("002" === deptCode || "013" === deptCode || "001" === deptCode || "QZB0014" === usercode) {
      allData = "allmess";
    }
    var token = "";
    ...
    if (request.custdoctype != undefined) {
      if ("1" == resultJSON.status && resultJSON.data != null) {
        ...
        var resa = ObjectStore.queryByYonQL(
          "select kongzhiweidu,zuzhi,bumen from GT90840AT64.GT90840AT64.djsjqxmx where djsjqxsz_id in (select id from GT90840AT64.GT90840AT64.djsjqxsz where danjumingchenMain='" +
            doctype +
            "') and renyuan='" +
            useridrole +
            "' "
