...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    let userInfos = openLinker("POST", "https://www.example.com/", "GT30661AT5", JSON.stringify(body));
    ...
    if (userRes.code == "200") {
      if (userRes.data.status == "1") {
        ...
        if (data.length > 0) {
          ...
          let staffurl = "https://www.example.com/" + id + "&code=" + code;
          let userDetail = openLinker("GET", staffurl, "GT30661AT5", JSON.stringify({}));
          let userDetailRes = JSON.parse(userDetail);
    ...
    let wres1 = workNotice.execute({ title: "分签协同申请", content: resultRet });
    return resultRes;
  }
}
