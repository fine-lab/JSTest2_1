...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    if (address == "yonbip_scm_storeprorecord_list" || address.indexOf("storeprorecord") > -1) {
      ...
      //期初库存（其他入库单的一种）
      sql =
        "select id, mainid.id mainId,mainid.org.name orgName,mainid.code billCode,mainid.vouchdate vouchDate,batchno,producedate,invaliddate,qty materialNum,unit.name unitName,product.cName materialName,product.id materialId from 		st.othinrecord.OthInRecords  where  1=1 ";
