...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    let func1 = extrequire("GT34544AT7.authManager.getAllXP");
    ...
    //出去重复
    function duplicateChecking(arr) {
      let temp = [];
      for (let item of arr) {
        item.children = [];
    ...
    function listToTree(list) {
      ...
      return list.filter((node) => {
        info[node.sys_parent] && info[node.sys_parent].children.push(node);
        return !node.sys_parent;
      });
    }
    ...
    let res2 = duplicateChecking(res1);
