...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    var Data = getOtherOutRecoeds([request.ID]);
    var bustype = Data[0].bustype;
    let DBAPIURL = extrequire("ST.rule.PublicGetURL");
    let DBorderBody = DBAPIURL.execute(null, "kwti8du9");
    if (bustype != "2369198631997957" && bustype != "1525755091611025411" && bustype != "1525755383641538567" && bustype != "1525755564049039368" && bustype != "1525755727257796612") {
      var _status = Data[0]["headItem!_status"];
      if (_status == "Insert" || _status == null) {
        // 获取销售出库单数据
        function getHed(Data) {
          var warehouse = Data[0].warehouse;
          var warehouse_name = Data[0].warehouse_name;
          ...
          let SNO = Data[0].hasOwnProperty("cLogisticsBillNo");
          if (SNO == true) {
            iLogisticId = Data[0].iLogisticId;
            if (null != iLogisticId && undefined != iLogisticId) {
              ...
              iLogisticId_name = Data[0].iLogisticId_name;
            }
            cLogisticsBillNo = Data[0].cLogisticsBillNo;
          }
          ...
          var vouchdate = new Date(Data[0].vouchdate);
    ...
    function getOtherOutRecoeds(ID) {
      var object = {
        ids: ID,
        compositions: [
          {
