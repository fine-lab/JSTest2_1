...
class MyAPIHandler extends AbstractAPIHandler {
  execute(request) {
    ...
    //不在循环拼接参数时更新，避免一条更新成功其他更新失败,一次更新，要么成功，要么失败
    //获取所有id
    ...
    ybresults.forEach((ybresult) => {
      ...
      saleresults.forEach((saleresult) => {
        ...
        if (saleresult.aftermoveaccmoney == 0) {
          return;
        }
      ...
      for (var key in custListMap) {
        ...
        //获取结算方式信息 20210531 添加，对应生成应收单和收款单的自定义项"结算方式1"
        var payTypeId = "";
        var payTypeName = "";
        ...
        if (key.indexOf("收款") != -1) {
          ...
          if (serviceAttr == 0 && account != "") {
            recbody = {
              ...
              tradetype: "2282612483346688",
              tradetype_name: "实际收款",
              tradetype_code: "YNWP01",
              customer_code: customer_code,
              exchRate: 1,
              exchangeRateType_code: "01",
              oriSum: income,
          ...
          } else if (serviceAttr != 0 && serviceAttr != 1 && account != "") {
            //结算方式和账号都不传
            recbody = {
              currency: "G001ZM0000DEFAULTCURRENCT00000000001",
              accentity_code: ybresult.erporgcode,
